...
namespace LFJ{
	class HomogeneousVolume : public AbstractObject{
		...
		float scale;
	public:
		HomogeneousVolume(Scene *scene, float g) : AbstractObject(scene){
	...
	private:
		vec3f tau(float dist) const{
			vec3f Tr; 
			for(int i = 0; i < 3; i++){
				Tr[i] = exp(-extinctionCoeff[i] * dist);
			}
			return Tr;
		}
		float pMedium(float s) const{
			vec3f pm;
			for(int i = 0; i < 3; i++){
				pm[i] = extinctionCoeff[i] * exp(-extinctionCoeff[i] * s);
			}
			return Luminance(pm);
		}
		float PSurface(float d) const{
			vec3f Pd;
			for(int i = 0; i < 3; i++){
				Pd[i] = exp(-extinctionCoeff[i] * d);
			}
			return Luminance(Pd);
		}
		...
		float sampleDist() const{
			return -log(1.0 - rng->genFloat()) / Luminance(extinctionCoeff);
		}
		float pDistance(float dist) const{
			return Luminance(extinctionCoeff) * exp(-Luminance(extinctionCoeff) * dist);
		}
	...
	public:
		void setSigma(vec3f sigmaS, vec3f sigmaA){
			scatteringCoeff = sigmaS * scale;
			absorptionCoeff = sigmaA * scale;
